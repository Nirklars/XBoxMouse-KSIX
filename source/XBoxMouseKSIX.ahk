...
if HaltProgram = 0 ; prevent looping overuse of cpu when no controller is connected
{
	...
	Hotkey, %JoystickPrefix%5, KeyKeyboard
	Hotkey, %JoystickPrefix%7, KeyTabPrev
	Hotkey, %JoystickPrefix%8, KeyTabNext
	Hotkey, %JoystickPrefix%11, KeyEscape
	Hotkey, %JoystickPrefix%12, KeyEnter
	Hotkey, %JoystickPrefix%14, SaveState
	Hotkey, %JoystickPrefix%15, LoadState
	
	; KSIX EDITS
	
	Hotkey, %JoystickPrefix%9, LeftTrigger
	Hotkey, %JoystickPrefix%10, RightTrigger
...
CheckAll:
	...
	if GetKeyState(JoystickPrefix . 8)
		RightModDown := 1
	else
		RightModDown := 0
	...
	if !GetKeyState(JoystickPrefix . 7) ;Hold down button 5 to proceed
		Return
	if !GetKeyState(JoystickPrefix . 8) ;Hold down button 6 to proceed
		Return
	...
	else if GetKeyState(JoystickPrefix . 5)
		{
			BlockPOVTab := 0
...
GoAltTab:
	...
	if A_OSVersion in WIN_XP,WIN_NT4,WIN_95,WIN_98,WIN_ME  ; Check if old OS is running, then the new alt tab wont work
	{
		...
		BlockPOVTab := 1
		Send {Alt down}
		KeyWait %JoystickPrefix%7
		KeyWait %JoystickPrefix%8
		Send {Alt up}
	}
	else
	{
...
KeyKeyboard:
	if (ToggleMouseSimulator = 1)
		return
	SetMouseDelay, -1  ; Makes movement smoother.
	if GetKeyState(JoystickPrefix . 7) ; ignore inputs while holding down this button
		{
		if GetKeyState(JoystickPrefix . 8) ; ignore inputs while holding down this button
			Return
		}
